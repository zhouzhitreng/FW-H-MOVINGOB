...
%% 给定积分面初始流场（单极子问题）
%% give parameter
outpp_name='dipole_tec_c0340_u0_000_t1.0.dat';
A=0.02;
rho0=1.0;
...
for j2=1:nump(2)
    for j1=1:nump(1)
        ...
        [ fi1,dfi_dt1,dfi_dx1,dfi_dy1 ] = calfi( t_plot,A,beta,w,M,k,node_plot(j,:)-delta_source);
        [ fi2,dfi_dt2,dfi_dx2,dfi_dy2 ] = calfi( t_plot,-A,beta,w,M,k,node_plot(j,:)+delta_source);
        pp_plot(j)=-rho0*(dfi_dt1+u0*dfi_dx1+dfi_dt2+u0*dfi_dx2)/dxsource;
        up_plot(j)=(dfi_dx1+dfi_dx2)/dxsource;
        vp_plot(j)=(dfi_dy1+dfi_dy2)/dxsource;
    end
end
